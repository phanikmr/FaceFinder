...
guidata(hObject, handles);set(handles.percentage,'String','WELCOME to FACEFINDER');
...
axes(handles.pie_chart),axis off; imshow('about.jpg');
...
h=errordlg('no faces found in database','File Error');   
beep;
uiwait(h);  
set(handles.inform_pane,'String','no faces found in database','FontSize',20.0,'BackgroundColor','red');
set(handles.percentage,'String','Go to help menu for help in saving images'); 
...
set(handles.inform_pane,'String','choose web_cam','FontSize',15.0)
delete(h);
catch
    delete(h);
    d=warndlg('no web_cam is installed');
    uiwait(d);
end
end
...
set(handles.inform_pane,'String','connecting to webcam.......','FontSize',15.0);
...
triggerconfig(vid_obj,'Manual');
 ...
 set(handles.inform_pane,'String',b,'FontSize',15.0);
 set(handles.percentage,'String','choose higher resolution format for better recognition');
 delete(d);
 set(gcf,'Pointer','arrow')
end
...
c=char(list_name(list_id));
 ...
 set(handles.inform_pane,'String',b,'FontSize',15.0)
 set(handles.percentage,'String','Ready to start Facefinding');
delete(d);
set(gcf,'Pointer','arrow')
end
...
a=sprintf('name :%s \nstatus: %s\nmatch no:1',image_info(1,reg_no).name,b);
...
a=sprintf('name :%s \nstatus: %s\nmatch no: %d',image_info(1,prediction(1,counter)).name,b,counter);
set(handles.inform_pane,'String',a);

catch
    counter=counter+1;
    set(handles.inform_pane,'String','matched faces over');
end
end
...
a=sprintf('name :%s \nstatus: %s\nmatch no: %d',image_info(1,prediction(1,counter)).name,b,counter);
set(handles.inform_pane,'String',a,'FontSize',15);
counter=counter+1;
catch
  %counter=counter-1;
...
x=warndlg('make sure to "run as adminisrator" or else the data may corrupt');
uiwait(x);
set(handles.inform_pane,'BackgroundColor',[0.73,0.83,0.96]);
...
if (strcmp(f,'type name')||(strcmp(f,''))==1)
    set(handles.inform_pane,'String','name of the face is empty')
else
...
a=sprintf('Train.no: %d \nname :%s \nstatus: %d\nsaved',Train_Number,f,get(handles.status,'Value'));
 set(handles.inform_pane,'String',a);
 
end
catch
    h=errordlg('"Access denied to write" make sure to "Run as administrator"');
uiwait(h);
end
end
...
try
    ...
    set(gcf,'Pointer','arrow')  ;
   set(handles.inform_pane,'String','unable to detect face in the image','FontSize',20.0 );
   h=warndlg('unable to detect face in the given image');
   uiwait(h);
end
  set(gcf,'Pointer','arrow')  
end
...
set(handles.inform_pane,'BackgroundColor',[0.73,0.83,0.96]);
    ...
    set(handles.start_button,'Visible','on');
    set(handles.stop_button,'Visible','on');
   set(handles.inform_pane,'String','choose web_cam')
   axes(handles.live_axis);axis off;
axes(handles.face_axis);axis off;
axes(handles.main_image1);axis off;
axes(handles.match_image1);axis off;
...
set(handles.webcam_image,'Checked','off');
    ...
    set(handles.stop_button,'Visible','off');
    set(handles.open,'Visible','on');
    set(handles.inform_pane,'String','choose image from open image button')
    axes(handles.live_axis);axis off;
axes(handles.face_axis);axis off;
axes(handles.main_image1);axis off;
axes(handles.match_image1);axis off;
...
try
try
    winopen('C:\Program Files\FACEFINDER\help\save.htm');
catch
     winopen('C:\Program Files(x86)\FACEFINDER\help\save.htm');
end
catch
    winopen('help\save.htm');
end
end
% --------------------------------------------------------------------
function webcam_help_Callback(hObject, eventdata, handles)
try
try
    winopen('C:\Program Files\FACEFINDER\help\webcam.htm');
catch
     winopen('C:\Program Files(x86)\FACEFINDER\help\webcam.htm');
end
catch
   winopen('help\webcam.htm');
end
end

% --------------------------------------------------------------------
function local_help_Callback(hObject, eventdata, handles)
try
 try
winopen('C:\Program Files\FACEFINDER\help\open.htm');
catch
     winopen('C:\Program Files(x86)\FACEFINDER\help\open.htm');
end
catch
    winopen('help\open.htm');
end
end
...
try
try
    winopen('C:\Program Files\FACEFINDER\help\tips.htm');
catch
     winopen('C:\Program Files(x86)\FACEFINDER\help\tips.htm');
end
catch
    winopen('help\tips.htm');
end
end
